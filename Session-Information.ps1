...
.NOTES
    ...
    2026/06/10  Guy Leech  Added host pool info
#>
...
else ## see if Azure
{
    ...
    ## TODO read host pool info and decode jwt (Get-ItemPropertyValue -Path "HKLM:\SOFTWARE\Microsoft\DeviceManageabilityCSP\Provider\WVD" -name ConfigInfo |convertfrom-json
    $hostPool = $null
    $hostPool = Get-ItemPropertyValue -path 'HKLM:\SOFTWARE\Microsoft\RDMonitoringAgent' -Name 'SessionHostPool' -ErrorAction SilentlyContinue ## TODO other things here like location lest imds fails
    if( -Not [string]::IsNullOrEmpty( $hostpool ))
    {
        ''
        "AVD Host Pool: $hostPool"
    }
}
...
if( $runFromExplorer )
{
    $null = Read-Host -Prompt "`nHit Enter to exit "
}
